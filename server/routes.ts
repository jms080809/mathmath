...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const problemsWithAuthor = await Promise.all(
        problems.map(async (problem) => {
          ...
          if (req.session.userId) {
            isSolved = await storage.checkIfProblemSolved(
              req.session.userId,
              problem.id
            );
            isSaved = await storage.checkIfProblemSaved(
              req.session.userId,
              problem.id
            );
          }
          ...
          return {
            problem: {
              ...
              createdAt:
                problem.createdAt instanceof Date
  ...
  app.get("/api/problems/recommend", requireAuth, async (req, res) => {
    try {
      ...
      // 최적화된 랜덤 미해결 문제 가져오기
      const randomProblems = await storage.getRandomUnsolvedProblems(
        userId,
        limit
      );
        ...
        if (problemData.type === "multiple-choice" && problemData.options) {
          try {
            ...
            if (typeof problemData.options === "string") {
              problemData.options = problemData.options
                .split(",")
  ...
  app.post("/api/problems/:id/check-answer", requireAuth, async (req, res) => {
    try {
      ...
      const alreadySolved = await storage.checkIfProblemSolved(
        userId,
        problemId
      );
      if (alreadySolved) {
        return res
          .status(400)
          .json({ message: "Problem already solved", correct: false });
      }
